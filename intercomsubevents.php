...
function returnEvents($data, $events) {
  foreach ($data->events as $event) {
    ...
    (
    ($event->event_name == 'clicked export')
    || ($event->event_name == 'clicked follow')
    || ($event->event_name == 'clicked unfollow')
    || ($event->event_name == 'created a report')
    || ($event->event_name == 'clicked create report')
    || ($event->event_name == 'clicked view more connected sources')
    || ($event->event_name == 'artist followed')
    || ($event->event_name == 'unfollowed artist')
    )
    )
    {
...
for($i=0;$i<count($user_IDs);$i++){
  $events = exhaustEventPages('https://api.intercom.io/events?type=user&_user_id=' . $user_IDs[$i], $events);
}
